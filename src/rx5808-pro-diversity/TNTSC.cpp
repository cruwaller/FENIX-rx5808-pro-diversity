...
//#define REALACC_RX5808_PRO_PLUS_OSD
  ...
  #define PWM_CLK PA9         // 同期信号出力ピン(PWM)
  #define DAT PB15             // 映像信号出力ピン
#endif
...
void TNTSC_class::begin(uint8_t mode, uint8_t spino, uint8_t* extram) {
  ...
  #endif
  #ifdef REALACC_RX5808_PRO_PLUS_OSD
      nvic_irq_set_priority(NVIC_TIMER1_CC, IRQ_PRIORITY); // 割り込み優先レベル設定
      Timer1.pause();                             // タイマー停止
      Timer1.setPrescaleFactor(NTSC_TIMER_DIV);   // システムクロック 72MHzを24MHzに分周 
      Timer1.setOverflow(1524);                   // カウンタ値1524でオーバーフロー発生 63.5us周期
  #endif
  ...
  #endif
  #ifdef REALACC_RX5808_PRO_PLUS_OSD
      timer_cc_set_pol(TIMER1,2,1);  // 出力をアクティブLOWに設定
  #endif
  ...
  #endif
  #ifdef REALACC_RX5808_PRO_PLUS_OSD
      Timer1.setCompare(1, 225-60+_hAdjust);  // オーバーヘッド分等の差し引き
      Timer1.setMode(1,TIMER_OUTPUTCOMPARE);
      Timer1.attachInterrupt(1, handle_vout);   
