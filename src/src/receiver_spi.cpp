...
static SPISettings DMA_ATTR spi_settings(1000000, LSBFIRST, SPI_MODE0);


namespace ReceiverSpi {

    static void IRAM_ATTR setPowerDownRegister(uint32_t value, Receiver::ReceiverId const ReceiverId)
    {
#ifdef DISABLE_AUDIO
    ...
    static void IRAM_ATTR setStateRegister(uint32_t const value, Receiver::ReceiverId const ReceiverId)
    {
    ...
    void IRAM_ATTR setSynthRegisterB(uint32_t const freq, Receiver::ReceiverId const ReceiverId)
    {
        uint32_t const value = converteFreqToSynthRegisterB(freq);
        sendRegister(SPI_ADDRESS_SYNTH_B, value, ReceiverId);
    }

    void IRAM_ATTR rxVideoOff(Receiver::ReceiverId const ReceiverId)
    {
    ...
    void IRAM_ATTR rxPowerOff(Receiver::ReceiverId const ReceiverId)
    {
        setPowerDownRegister(0b11111111111111111111, ReceiverId);
    }

    void IRAM_ATTR rxPowerUp(Receiver::ReceiverId const ReceiverId)
    {
    ...
    void IRAM_ATTR rxStandby(Receiver::ReceiverId const ReceiverId)
    {
        //setStateRegister(RX5808_STATE_VCO_CAL, ReceiverId);
        setStateRegister(RX5808_STATE_STBY, ReceiverId);
    }

    void IRAM_ATTR rxWakeup(Receiver::ReceiverId const ReceiverId)
    {
...
sendRegister(uint8_t const addressBits, uint32_t const dataBits,
             const Receiver::ReceiverId rx_id)
{
    // LSB first
    uint32_t const data = addressBits | (0x1 << 4) | (dataBits << 5);
...
converteFreqToSynthRegisterB(uint32_t const freq)
{
    //
